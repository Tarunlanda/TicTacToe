...
class Nakama {
    ...
    constructor() {
        const port = process.env.NEXT_PUBLIC_SERVER_PORT || "7350";
    ...
    async findMatch(nickname: string = '', sessionId: string = ''): Promise<void> {
        ...
        const payload = nickname ? { nickname, sessionId } : {};
        const matches = await this.client.rpc(this.session, rpc_name, payload);
